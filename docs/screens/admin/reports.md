...
Note: the *price-review* triage queue is a separate screen (`/admin/price-review`) even though its client logic also lives in `ReportsService` — see [price-review.md](./price-review.md). The consignment seller-payouts view briefly lived here as a 6th tab (2026-07-14) but moved the same day to the per-seller screen `/admin/sellers/:id` — see [seller-detail.md](./seller-detail.md).
...
- [sellers.md](./sellers.md) / [seller-detail.md](./seller-detail.md) — consignment sellers CRUD + the per-seller payouts screen
