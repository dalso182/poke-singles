...
| Prod | `poke-singles.com` (**live since 2026-07-22**) | `dhslfridsjdmhwzrgebv` (the original project, promoted) | Pro org |
...
4. **The backup guard:** `assertRemoteAllowed()` rejects any remote dir matching `BLOCKED_REMOTE_PATHS = [/(^|\/)poke-singles\.com\/public_html_opencart\/?$/i]` — the parked OpenCart backup (renamed at the 2026-07-22 swap). The live root itself is now the legitimate `deploy:prod` target, with real creds in `.env.local`.
...
- **`scripts/fetch-card-images.mjs`** (`npm run images:fetch`) downloads TCGdex card scans into a local `./card-images/<serie>/<set>/<localId>.<ext>` tree. Flags: `--out=./card-images`, `--sets=a,b` (the "new set dropped" path), `--series=a,b`, `--quality=high|low` (default high), `--ext=webp|png|jpg` (default webp), `--lang=en`, `--concurrency=8`, `--logos` (set logos + symbols), `--dry-run`. Resumable (existing non-empty files skipped; `.part` + rename prevents truncated files); cards with no contributed scan are recorded in `card-images/missing-images.json` — **except the five SWSH gallery subsets** (`PTCGIO_FALLBACK_SETS`: `swsh9.5tg`, `swsh11.5tg`, `swsh12.5tg`, `swsh12.5gg`, `swsh4.5sv`), which TCGdex has no scans for: those download from `images.pokemontcg.io/<mappedSet>/<localId>_hires.png` and are transcoded to `--ext` via `sharp` (devDependency, lazily imported), landing at the same TCGdex-id path. Summary lines report fallback counts separately. Never touches Supabase.
...
- **URL helpers** (`src/app/core/images/card-image-url.ts`): `tcgdexImageToHostedPath()` maps a TCGdex asset URL (`https://assets.tcgdex.net/en/swsh/swsh3/136`) to the hosted relative path (`/card-images/swsh/swsh3/136.webp`, prefix constant `CARD_IMAGES_PREFIX = '/card-images'`); `hostedCardImagePath(serieId, setId, localId)` builds the same path from ids for cards with no TCGdex image URL (gallery subsets — the serie id comes from a set lookup, it's not on the card payload); `resolveHostedSrc()` absolutizes relative values against an origin for previews.
- **Localhost:** `proxy.conf.mjs` forwards `/card-images` from `ng serve` to `https://dev.poke-singles.com`, attaching HTTP Basic credentials from `.env.local` (`IMAGES_HTTP_USER`, `IMAGES_HTTP_PASSWORD` — the dev site is site-password-protected). Missing creds just yield 401s on images locally, with a console warning. It must stay pointed at `dev.` — that host's `auth-config.php` is stamped for the dev Supabase project, so local admin tokens validate; the retiring `new.` host still carries pre-split endpoints that reject them with 401 `invalid_token`.
...
- **The OpenCart backup folder is protected**: the `BLOCKED_REMOTE_PATHS` regex in `deploy.mjs` refuses deploys into `public_html_opencart`. Never deploy into it; it's the rollback until decommission.
- **Image paths are relative by contract** (`/card-images/...`) — code that writes `products.image_url` from TCGdex data must go through `tcgdexImageToHostedPath()` (or `hostedCardImagePath()` when the card has no image URL), and the upload script may only write inside a `/card-images` directory.
...
- **Cutover happened 2026-07-22**: staged deploy to `public_html_new` + atomic two-rename swap (`public_html` → `public_html_opencart`, `public_html_new` → `public_html`) with maintenance mode ON. Rollback = the two renames reversed over SSH. The site launched with an empty catalog (restock via `/admin/products/new`); OC lifetime totals live in `app_settings.legacy_*`, `orders_number_seq` continued from 7304.
- The deploy guard regex only blocks paths **ending** in `poke-singles.com/public_html_opencart` (optional trailing slash) — the live root is a legitimate target now.
- `npm run deploy` (no suffix) targets **prod** — the live site. Use `deploy:dev` habitually.
...
- `proxy.conf.mjs` only affects `ng serve`; a deployed dev build never proxies. It targets `dev.poke-singles.com` (switched 2026-07-14 — the old `new.` target's stale pre-split PHP gate broke the local image picker with 401 `invalid_token`). If local images 401, set `IMAGES_HTTP_USER`/`IMAGES_HTTP_PASSWORD` and restart `npm start` (do not kill the owner's running server on port 4242).
- `.env.local.example` shows `DEPLOY_REMOTE_DIR=/home/customer/www/poke-singles.com/public_html` — post-cutover that IS the correct prod target.
