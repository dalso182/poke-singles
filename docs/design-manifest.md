...
| `app-checkbox` | Plain square checkbox (e.g. Productos "Destacado", Consignaciones selection) | `on: boolean` (model) = `false`; `disabled: boolean` = `false`; `indeterminate: boolean` = `false` (dash + `aria-checked="mixed"` while off — select-all half-state) | `change: boolean` | none | `<button role="checkbox">` + `mat-icon` |
