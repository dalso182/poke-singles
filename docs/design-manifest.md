...
| `app-checkbox` | Plain square checkbox (e.g. Productos "Destacado", seller-detail payout selection) | `on: boolean` (model) = `false`; `disabled: boolean` = `false`; `indeterminate: boolean` = `false` (dash + `aria-checked="mixed"` while off — select-all half-state) | `change: boolean` | none | `<button role="checkbox">` + `mat-icon` |
...
| `app-bulk-bar` | Bulk-action toolbar shown while rows are selected (blue-bordered card: "N seleccionados" + "Limpiar" + projected actions); parent gates with `@if (count > 0)` | `count: number` (required) | `clear: void` | default (actions, after the spacer) | no |
