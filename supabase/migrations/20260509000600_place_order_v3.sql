-- place_order v3: backfill profile name / phone / default_shipping_address
-- from the order's buyer info, but only when the existing field is empty.
-- coalesce(nullif(value, ''), v_*) preserves any value the user already set
-- on /account, so this is "fill the blanks" not "overwrite". Address backfill
-- is null-only since jsonb addresses don't have a meaningful empty literal.
-- Logic is otherwise identical to v2.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(v_items) loop
    ...
    where id = (v_item ->> 'product_id')::uuid;
    -- Set name snapshot — null if the product has no set assigned.
    v_set_name := null;
