-- place_order v11: refuse auction products in the cart.
--   Identical to v10 EXCEPT the item-lock loop rejects any product in the
--   Subastas category with AUCTION_NOT_PURCHASABLE. Without this, anyone could
--   buy an auctioned card at its starting price through the normal checkout
--   (auction products are visible under products_public_read even at
--   quantity 0/1, and the cart API takes any product id). Auction cards are
--   sold exclusively through place_bid + the auction close flow, which creates
--   the winner's order server-side.
...
begin
  ...
  -- v9: when the method declares an allow-list, every distinct category in
  -- the cart must appear in it. Empty allow-list = all categories pass.
  if array_length(v_shipping.allowed_category_ids, 1) is not null then
    select array_agg(distinct p.category_id)
      into v_cart_cats
      from jsonb_array_elements(v_items) e(value)
      join public.products p
        on p.id = (e.value ->> 'product_id')::uuid;
    if v_cart_cats is not null
       and not (v_cart_cats <@ v_shipping.allowed_category_ids) then
      return jsonb_build_object('ok', false, 'error', 'SHIPPING_NOT_ALLOWED_FOR_CART');
    end if;
  end if;
  ...
  -- Load the coupon up front so its category scope is known during the item
  -- loop. Locked FOR UPDATE to serialize redemption counting.
  ...
  -- Lock products in a fixed global order (ascending product_id) so concurrent
  -- overlapping carts can never form a lock cycle (v7 deadlock fix).
  for v_item in
    select e.value
    from jsonb_array_elements(v_items) e(value)
    order by (e.value ->> 'product_id')::uuid
  loop
    ...
    -- v11: auction products are never sold through the cart — only through
    -- place_bid + the auction close flow.
    if v_product.category_id = public.auction_category_id() then
      return jsonb_build_object(
        'ok', false, 'error', 'AUCTION_NOT_PURCHASABLE',
        'product_id', v_product.id
      );
    end if;
    ...
    if v_coupon.id is not null
       and (not v_targeted or v_product.category_id = any (v_coupon.category_ids)) then
      v_eligible := v_eligible + (v_unit_price * v_qty);
    end if;
  end loop;

  -- Coupon checks + discount, all against the eligible subtotal.
  if v_coupon.id is not null then
    if v_targeted and v_eligible <= 0 then
      return jsonb_build_object('ok', false, 'error', 'COUPON_NO_ELIGIBLE');
    end if;
    ...
    if v_coupon.type = 'PERCENTAGE' then
      v_discount := round(v_eligible * v_coupon.discount_value / 100, 2);
    elsif v_coupon.type = 'FIXED_ON_THRESHOLD' then
      v_discount := v_coupon.discount_value;
    end if;
    if v_discount > v_eligible then v_discount := v_eligible; end if;
  end if;
  ...
  for v_item in select * from jsonb_array_elements(v_items) loop
    ...
    -- v10: consignment snapshot. House products leave all three NULL.
    v_seller_code := null;
    v_seller_name := null;
  ...
  -- v8: record the order on the customer activity log (IP from request headers).
  insert into public.customer_activity (
    user_id, customer_name, customer_email, event_type, order_id, ip
  ) values (
    v_uid, v_name, v_email, 'order_created', v_order_id, public.client_ip()
  );
