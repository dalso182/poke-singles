-- place_order v8: log an 'order_created' customer-activity event.
--   Identical to v7 (deterministic product lock order) EXCEPT it inserts one
--   public.customer_activity row just before returning, recording who placed the
--   order, the order id, and the client IP (public.client_ip(), captured from the
--   browser's forwarded request headers since checkout calls this RPC directly).
--   This is the order side of the admin Customer Activity Report; login /
--   registered events are logged client-side via log_activity().
--
--   Logged for both guests (v_uid null) and signed-in buyers — the report shows
--   the snapshotted name/email regardless. Same transaction as the order, so a
--   rolled-back checkout leaves no orphan activity row.
