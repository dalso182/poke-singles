-- place_order v6: category-targeted coupons (builds on v5's sale_price logic).
--   - The coupon row is now loaded BEFORE the item loop so its category_ids
--     allow-list is known while we walk the cart.
--   - The item loop accumulates an "eligible" subtotal (items whose category
--     is covered by the coupon) alongside the full subtotal, both priced at
--     the effective price coalesce(sale_price, price) — unchanged from v5.
--   - Discount + the FIXED_ON_THRESHOLD minimum are computed against the
--     eligible subtotal; the discount is capped at it.
--   - New error COUPON_NO_ELIGIBLE when a targeted coupon matches no cart item.
-- Everything else (shipping, address, stock, redemptions, profile backfill,
-- sale_price pricing) is identical to v5.
