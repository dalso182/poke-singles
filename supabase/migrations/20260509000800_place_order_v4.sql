-- place_order v4: honor shipping_methods.requires_address.
--   - When the chosen method requires an address, validate line1/city/province
--     non-empty and return ADDRESS_REQUIRED on miss.
--   - When it doesn't, NULL out the buyer address before insert so pickup
--     orders never carry phantom address data.
-- Profile-backfill block stays as-is — coalesce(default_shipping_address, v_address)
-- is correct: if v_address is null (pickup), no overwrite, no fill.
-- Logic is otherwise identical to v3.
...
begin
  ...
  -- Address validation depends on the chosen method.
  if v_shipping.requires_address then
    ...
    -- Pickup-style method — discard whatever the client sent so the order
    -- doesn't carry stale/empty address fields.
    v_address := null;
  end if;
  ...
  if v_uid is not null then
    ...
    -- Backfill profile fields that are still empty so future checkouts
    -- prefill correctly. Existing non-empty values are preserved.
