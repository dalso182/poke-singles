-- place_order v9: enforce per-method category allow-list at checkout time.
--   Identical to v8 EXCEPT, right after locking the chosen shipping_methods
--   row, we resolve the cart's distinct category_ids and reject the order if
--   the method declares an allow-list that doesn't cover every category in the
--   cart. An empty allowed_category_ids array (the default) preserves the
--   prior any-cart behavior, so existing methods need no migration.
--
--   Error code: SHIPPING_NOT_ALLOWED_FOR_CART (mapped client-side to a
--   friendly Spanish message). The check runs before stock locks, so a
--   rejection is cheap and leaves no half-applied state.
