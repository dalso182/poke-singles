-- place_order v7: deterministic product lock order (deadlock fix).
--   Identical to v6 in every respect EXCEPT the first item loop now acquires the
--   per-product FOR UPDATE locks in a fixed global order (ascending product_id)
--   instead of cart-array order.
--
--   Why: v6 locked products in the order they appeared in the cart, which differs
--   between buyers. Two simultaneous multi-item checkouts sharing >=2 products in
--   opposite order could deadlock (SQLSTATE 40P01) or pile up into lock-wait
--   statement timeouts. Locking every product in the same order across all
--   transactions makes a lock cycle impossible, so concurrent overlapping carts
--   simply queue instead of deadlocking. (Overselling was already prevented by
--   the FOR UPDATE + single-transaction + check(quantity >= 0); unchanged.)
--
--   The second loop (order_items insert + stock decrement) is unchanged: those
--   rows are already locked from the first loop, so its iteration order is moot.
--   The coupon is still locked FOR UPDATE before the product loop, so the global
--   order stays coupon -> products-ascending for every transaction.
