-- place_order v10: snapshot the consignment seller onto each order item.
--   Identical to v9 EXCEPT the item-insert loop also resolves the product's
--   seller (products.seller_id → sellers) and writes seller_id / seller_code /
--   seller_name onto order_items, so consignment attribution survives later
--   product edits, product deletion, and even seller-row deletion. House
--   inventory (seller_id IS NULL, i.e. Poke-Singles) writes NULLs, exactly as
--   every pre-v10 order item already looks.
