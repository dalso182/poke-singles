-- Expose the card condition (NM/LP/MP/HP/DMG) on the public raffle listing so the
-- /rifas card can show a condition pill, matching the product card. A raffle prize
-- is a single card, so its condition matters to buyers. Appended at the end
-- (CREATE OR REPLACE VIEW only allows adding columns last). Existing raffles with a
-- null condition simply render no pill.
