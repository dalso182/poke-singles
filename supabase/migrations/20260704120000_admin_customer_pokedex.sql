-- Surface the customer's Pokédex collection on the admin detail RPC so
-- /admin/customers/:id can render the same Pokédex view the customer sees in
-- /account. profiles.caught_pokemon_numbers is integer[] NOT NULL DEFAULT '{}'
-- (see 20260630000000), so no coalesce is needed — to_jsonb turns it into a
-- plain JSON number array. jsonb-returning → CREATE OR REPLACE is fine.
